...
class TestTheBestDealsStoresContent(BaseCase):
    ...
    def test_list_of_store_content(self):
        ...
        list_no = len(self.find_elements("div.bg amp-carousel[height='101'] div.i-amphtml-carousel-scroll > a"))
        print(list_no)

        # print all the stores name
        for i in range(list_no):
            # get all the stores by name
            stores = self.find_element(
            ...
            stores_alt_attr = self.find_element(
                f"amp-carousel[height='101'] div.i-amphtml-carousel-scroll > a:nth-child({i ++1}) > amp-img"
            ).get_attribute('alt')
            print(stores_alt_attr)
            # get the div content
            stores_content = self.find_element(
                f"amp-carousel[height='101'] div.i-amphtml-carousel-scroll > a:nth-child({i ++1}) > div").text
